...
def apply_strategy(districts, week, effort):
    p1effort = player1_allocate(districts, effort)
    p2effort = player2_allocate(districts, effort)
...
def player1_allocate(polls, effort):
    p1allocation = strategies.strat_x(polls, effort)
    return p1allocation
...
def player2_allocate(polls, effort):
    p2pov = flip_list(polls)
    p2allocation = strategies.strat_y(p2pov, effort)
